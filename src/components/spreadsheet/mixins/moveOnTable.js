  ...
  methods: {
    ...
    moveKeydown (event) {
      ...
      if (
        ...
        const currentlyEditingCell = this.data[rowIndex][header].show

        if (!currentlyEditingCell) {
          if (!this.setFirstCell) {
            this.$set(this.data[rowIndex][header], 'rectangleSelection', true)
            this.setFirstCell = true
          }

          // set colMax rowMax
          const rowMax = this.data.length
          ...
          if (this.keys[16]) {
            ...
            this.$set(this.data[rowIndex][header], 'active', false)
          ...
          if (event.keyCode === 13) {
            if (this.$refs[`input-${this.customTable}-${colIndex}-${rowIndex}`]) {
              this.data[rowIndex][header].show = true
          ...
          // press esc
          if (event.keyCode === 27) {
            this.data[rowIndex][header].active = false
            this.storeCopyDatas = []
    ...
    moveOnSelect (event) {
      ...
      if (event.keyCode === 13) {
        ...
        const currentSelect = this.data[oldSelect.rowIndex][oldSelect.header]
    ...
    pressShiftMultipleCell (event, h, rowMax, rowIndex, colMax, colIndex) {
      ...
      this.$set(this.data[this.incrementRow][header], 'active', true)
    ...
    handleTbodyNavBackspace (rowIndex, colIndex, header) {
      if (this.selectedMultipleCell) {
        ...
        const cell = this.data[rowIndex][header]
    ...
    updateSelectedCell (header, rowIndex, colIndex) {
      ...
      this.data[rowIndex][header].stateCopy = false
      cleanProperty(td)
